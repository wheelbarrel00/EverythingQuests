...
-- [local patch: HBD released-pin nudging guard]
-- worldmapPinsPool.resetFunc clears pin.owningMap, but Blizzard's
-- MapCanvas UpdatePinNudging -> SetNudgeVector -> ApplyCurrentPosition can
-- still run against a released pin, where self:GetMap() is now nil and
-- MapCanvas_DataProviderBase.lua:366 errors. Upstream HBD-Pins (MINOR 17)
-- has not fixed this. Bail out when the pin is no longer bound to a map.
function worldmapProviderPin:ApplyCurrentPosition()
	if not self:GetMap() then return end
	MapCanvasPinMixin.ApplyCurrentPosition(self)
end
